import { useState, useEffect } from 'react';

// 1. Required NavBar Component
const NavBar = () => {
  return (
    <nav style={{ display: 'flex', gap: '30px', alignItems: 'center' }}>
      <a href="#home" style={{ color: '#333', textDecoration: 'none', fontSize: '18px', fontWeight: '500' }}>Home</a>
      <a href="#profile" style={{ color: '#333', textDecoration: 'none', fontSize: '18px', fontWeight: '500' }}>Profile</a>
      <a href="#circle" style={{ color: '#333', textDecoration: 'none', fontSize: '18px', fontWeight: '500' }}>My Circle</a>
    </nav>
  );
};

// 2. Additional Component: ProfileCard
const ProfileCard = ({ name, email, bio, interests, imageUrl }: any) => {
  const [isFavorite, setIsFavorite] = useState(false);

  return (
    <div style={{
      backgroundColor: '#4a4a4a',
      borderRadius: '15px',
      overflow: 'hidden',
      display: 'flex',
      flexDirection: 'column',
      height: '100%'
    }}>
      <div style={{ position: 'relative', padding: '20px', textAlign: 'center' }}>
        <button 
          onClick={() => setIsFavorite(!isFavorite)}
          style={{ 
            position: 'absolute',
            top: '15px',
            right: '15px',
            backgroundColor: isFavorite ? '#ff4757' : 'rgba(0,0,0,0.3)',
            border: 'none',
            color: 'white',
            cursor: 'pointer',
            fontSize: '28px',
            width: '40px',
            height: '40px',
            borderRadius: '5px',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            transition: 'background-color 0.3s'
          }}
        >
          {isFavorite ? '♥' : '+'}
        </button>
        <div style={{ width: '140px', height: '140px', margin: '0 auto 15px' }}>
          <img src={imageUrl} alt={name} style={{
            width: '100%',
            height: '100%',
            borderRadius: '50%',
            border: '4px solid #555',
            objectFit: 'cover'
          }} />
        </div>
        <div style={{ color: 'white', fontSize: '20px', marginBottom: '15px' }}>{name}</div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', flexWrap: 'wrap' }}>
          {interests.map((interest: string, index: number) => (
            <span key={index} style={{
              backgroundColor: '#888',
              color: 'white',
              padding: '6px 16px',
              borderRadius: '15px',
              fontSize: '13px'
            }}>{interest}</span>
          ))}
        </div>
      </div>
      <div style={{
        backgroundColor: '#d4d4d4',
        padding: '20px',
        color: '#000',
        flexGrow: 1,
        display: 'flex',
        flexDirection: 'column'
      }}>
        <div style={{ fontSize: '13px', marginBottom: '12px' }}>{email}</div>
        <div style={{ fontSize: '14px', lineHeight: '1.5', marginTop: '5px' }}>{bio}</div>
      </div>
    </div>
  );
};

// 3. Main App Component
function App() {
  const [searchTerm, setSearchTerm] = useState("");

  const allProfiles = [
    {
      name: "Breeze",
      email: "connect@chrisbrown.com",
      bio: "I am a multi-platinum R&B singer, dancer, and actor who first rose to global prominence as a teenager.",
      interests: ["Music", "Dance", "Nature"],
      imageUrl: "https://ntvb.tmsimg.com/assets/assets/499496_v9_bc.jpg"
    },
    {
      name: "Miky",
      email: "connect@michrealjackson.com",
      bio: "I am the 'King of Pop,' a global cultural icon who redefined the music industry through my groundbreaking albums like Thriller.",
      interests: ["Music", "Dance", "Video Games"],
      imageUrl: "https://impro.usercontent.one/appid/oneComWsb/domain/jackson-source.com/media/jackson-source.com/onewebmedia/Michael%20Jackson%202008.jpg?etag=%223c5a8c-60cf06bd%22&sourceContentType=image%2Fjpeg&ignoreAspectRatio&resize=1554%2B2071&extract=508%2B32%2B840%2B840&quality=85"
    },
    {
      name: "Will smithy",
      email: "connect@willsmith.com",
      bio: "I am an Academy Award-winning actor and Grammy-winning rapper, who transitioned from 'The Fresh Prince of Bel-Air' to Hollywood.",
      interests: ["Acting", "Tennis", "Reading"],
      imageUrl: "https://goldenglobes.com/wp-content/uploads/2023/10/will-smith-c-hfpa-2016.jpg?w=600"
    }
  ];

  const filteredProfiles = allProfiles.filter(profile => 
    profile.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  useEffect(() => {
    console.log("Current Search Term:", searchTerm);
  }, [searchTerm]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', fontFamily: 'Georgia, serif', backgroundColor: '#f5f5f5' }}>
      <header style={{
        backgroundColor: '#e8e8e8',
        padding: '20px 40px',
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
        boxShadow: '0 2px 5px rgba(0,0,0,0.1)'
      }}>
        <div style={{
          display: 'flex',
          alignItems: 'center',
          backgroundColor: '#a8a8a8',
          borderRadius: '25px',
          padding: '5px 5px 5px 20px',
          minWidth: '350px'
        }}>
          <input 
            type="text" 
            placeholder="What are you looking for?" 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            style={{
              background: 'transparent',
              border: 'none',
              color: 'white',
              fontStyle: 'italic',
              fontSize: '16px',
              width: '100%',
              outline: 'none'
            }}
          />
          <button 
            aria-label="Search"
            style={{
              backgroundColor: '#333',
              border: 'none',
              borderRadius: '50%',
              width: '36px',
              height: '36px',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              cursor: 'pointer',
              marginLeft: '10px',
              padding: 0,
              flexShrink: 0
            }}
          >
            <svg 
              width="20" 
              height="20" 
              viewBox="0 0 24 24" 
              fill="none" 
              stroke="white" 
              strokeWidth="2.5"
              strokeLinecap="round" 
              strokeLinejoin="round"
              style={{ display: 'block' }}
            >
              <circle cx="11" cy="11" r="8" />
              <line x1="21" y1="21" x2="16.65" y2="16.65" />
            </svg>
          </button>
        </div>
       
        <NavBar />

        <div style={{ display: 'flex', gap: '15px' }}>
          <button style={{
            backgroundColor: 'transparent',
            border: '2px solid #333',
            padding: '10px 24px',
            borderRadius: '25px',
            cursor: 'pointer',
            fontSize: '16px',
            fontWeight: '500',
            color: '#000'
          }}>Sign up</button>
          <button style={{
            backgroundColor: '#333',
            color: 'white',
            border: '2px solid #333',
            padding: '10px 24px',
            borderRadius: '25px',
            cursor: 'pointer',
            fontSize: '16px',
            fontWeight: '500'
          }}>Login</button>
        </div>
      </header>

      <main style={{
        flex: 1,
        maxWidth: '1400px',
        margin: '60px auto',
        padding: '0 40px',
        width: '100%'
      }}>
        <h1 style={{ textAlign: 'center', fontSize: '42px', marginBottom: '60px', color: '#333' }}>Find Your Match</h1>
        <div style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(3, 1fr)',
          gap: '40px',
          marginBottom: '80px'
        }}>
          {filteredProfiles.length > 0 ? (
            filteredProfiles.map((profile, index) => (
              <ProfileCard key={index} {...profile} />
            ))
          ) : (
            <p style={{ textAlign: 'center', color: '#666', gridColumn: '1 / -1' }}>
              No profiles found matching "{searchTerm}"
            </p>
          )}
        </div>
      </main>

      <footer style={{
        backgroundColor: '#3a3a3a',
        padding: '40px',
        textAlign: 'center'
      }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '30px' }}>
          <a href="#instagram" style={{
            width: '50px',
            height: '50px',
            borderRadius: '10px',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            background: 'linear-gradient(45deg, #f09433, #bc1888)'
          }}>
            <svg viewBox="0 0 24 24" width="28" height="28" fill="white"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
          </a>
          <a href="#twitter" style={{
            width: '50px',
            height: '50px',
            borderRadius: '10px',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            backgroundColor: '#000'
          }}>
            <svg viewBox="0 0 24 24" width="28" height="28" fill="white"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          </a>
          <a href="#youtube" style={{
            width: '50px',
            height: '50px',
            borderRadius: '10px',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            backgroundColor: '#ff0000'
          }}>
            <svg viewBox="0 0 24 24" width="28" height="28" fill="white"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
          </a>
        </div>
      </footer>
    </div>
  );
}

export default App;